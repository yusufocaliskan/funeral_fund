...
(function ($, window, document, undefined) {
  ...
  $win.on("load", function () {
    ...
    $("body").on("click", ".addCustomTimeslot", function (e) {
      e.preventDefault();
      var thisTimeslot = $(".booked-customTimeslotTemplate")
        .clone()
        .appendTo(timeslotsContainter)
        .removeClass()
        .addClass("booked-customTimeslot")
        .show();
      var rand = Math.floor(Math.random() * 100000000 + 1);
      init_custom_timeslot_block(thisTimeslot, rand);
    });

    var preventApptsBefore = $("body").find(
      ".booked_prevent_appointments_before"
    );
    var preventApptsAfter = $("body").find(
    ...
    preventApptsBefore.datepicker({
      ...
      onSelect: function (selected) {
        $(".booked_prevent_appointments_after").datepicker(
          "option",
          "minDate",
          selected
        );
        booked_datepicker_show_formatted_date(
          "booked_prevent_appointments_before",
          selected
        );
      },
    ...
    preventApptsAfter.datepicker({
      ...
      onSelect: function (selected) {
        $(".booked_prevent_appointments_before").datepicker(
          "option",
          "maxDate",
          selected
        );
    ...
    $("body").on("change", 'select[name="booked_booking_type"]', function () {
      ...
      if (thisSelectVal == "guest") {
        var selectedRadio = $(
          'input[name="booked_appointment_redirect_type"]:checked'
        ).val();
        if (selectedRadio == "booked-profile") {
          $('input[name="booked_appointment_redirect_type"][value=""]').prop(
            "checked",
            true
          );
        }
    ...
    $("body").on(
      "change",
      "#booked_hide_unavailable_timeslots,#booked_public_appointments",
      function () {
        var thisCheckbox = $(this);
        var thisID = thisCheckbox.attr("id");
        if (
          thisCheckbox.is(":checked") &&
          thisID == "booked_hide_unavailable_timeslots"
        ) {
          $("#booked_public_appointments").prop("checked", false);
        } else if (
          thisCheckbox.is(":checked") &&
          thisID == "booked_public_appointments"
        ) {
    ...
    $("body").on(
      "click",
      "form#newAppointmentForm input[type=submit]",
      function (e) {
        e.preventDefault();

        var thisForm = $("#newAppointmentForm"),
          customerType = thisForm
            .find("input[name=customer_type]:checked")
            .val(),
          ...
          guest_surnameActive = thisForm.find(
        ...
        if (
          ...
          $thisButton
            .val(booked_js_vars.i18n_please_wait)
            .attr("disabled", true);
          $thisButton
            .parents("form")
            .find("button.cancel")
          ...
          booked_js_vars.ajaxRequests.push = $.bookedAjaxQueue({
            ...
            success: function (date) {
              ...
              if (data[0] == "error") {
                $thisButton
                  .val(booked_js_vars.i18n_create_appointment)
                  .attr("disabled", false);
                $thisButton
                  .parents("form")
                  .find("button.cancel")
                ...
                $.ajax({
                  ...
                  type: "post",
                  data: {
                    action: "booked_admin_refresh_date_square",
                    nonce: booked_js_vars.nonce,
                    date: data[1],
                    calendar_id: calendar_id,
                  },
        ...
        if (
          (customerType == "new" && name && email && !surnameActive) ||
          (customerType == "new" && name && email && surnameActive && surname)
        ) {
          $("form.booked-form input").each(function () {
            thisDefault = $(this).attr("title");
            thisVal = $(this).val();
    ...
    $("body").on(
      "click",
      "form#editAppointmentForm input[type=submit]",
      function (e) {
        e.preventDefault();
        ...
        booked_js_vars.ajaxRequests.push = $.bookedAjaxQueue({
          ...
          success: function (result) {
            ...
            if (data[0] == "error") {
              $thisButton
                .val(booked_js_vars.i18n_update_appointment)
                .attr("disabled", false);
              alert(data[1]);
    ...
    function init_custom_timeslot_block(thisTimeslot, rand) {
      ...
      thisTimeslot
        .find("#vacationDayCheckbox")
        .attr("id", "vacationDayCheckbox-" + rand);
      thisTimeslot
        .find('label[for="vacationDayCheckbox"]')
        .attr("for", "vacationDayCheckbox-" + rand);

      var disableApptsValue = thisTimeslot
        .find("#vacationDayCheckbox-" + rand)
        .is(":checked");
      if (disableApptsValue) {
        hideAddCustomTimeslotsForm();
        thisTimeslot
          .find(
            "button.addSingleTimeslot,button.addBulkTimeslots,.customTimeslotsList"
          )
          .hide();
      } else {
      ...
      $("#booked-customTimePickerTemplates").on(
        "click",
        "button.cancel",
        function (e) {
          e.preventDefault();
      ...
      thisTimeslot.on("click", ".deleteCustomTimeslot", function (e) {
        ...
        if (confirmDelete) {
          ...
          $("#booked-customTimePickerTemplates")
            .hide()
            .appendTo("#booked-custom-timeslots");
          $("button.addBulkTimeslots").prop("disabled", false);
          $("button.addSingleTimeslot").prop("disabled", false);
      ...
      thisTimeslot.on("change", "input#all_day_custom", function () {
        ...
        if (thisCheckbox.is(":checked")) {
          thisCheckbox
            .parents(".customSingle")
            .find('select[name="startTime"]')
            .val("0000")
            .hide();
          thisCheckbox
            .parents(".customSingle")
            .find('select[name="endTime"]')
            .val("2400")
            .hide();
        } else {
          thisCheckbox
      ...
      thisTimeslot.on("click", ".customTimeslotsList .delete", function (e) {
        ...
        if (confirmDelete) {
          ...
          (deleteTimeslot = thisButton.parent().attr("data-timeslot")),
            (calendar_id = thisButton
              .parents(".booked-customTimeslot")
              .find('[name="booked_custom_calendar_id"]')
              .val()),
            (start_date = thisButton
              .parents(".booked-customTimeslot")
              .find('[name="booked_custom_start_date"]')
              .val()),
            (end_date = thisButton
              .parents(".booked-customTimeslot")
              .find('[name="booked_custom_end_date"]')
              .val()),
            (currentTimesBox = thisButton
              .parents(".booked-customTimeslot")
              .find("input[name=booked_this_custom_timelots]")),
            (currentArray = thisButton
              .parents(".booked-customTimeslot")
              .find("input[name=booked_this_custom_timelots]")
              .val()),
            (currentTimesBoxDetails = thisButton
              .parents(".booked-customTimeslot")
              .find("input[name=booked_this_custom_timelots_details]")),
            (currentArrayDetails = thisButton
              .parents(".booked-customTimeslot")
              .find("input[name=booked_this_custom_timelots_details]")
              .val()),
            (timeslotList = thisButton
              .parents(".booked-customTimeslot")
              .find(".customTimeslotsList"));
          ...
          booked_js_vars.ajaxRequests.push = $.bookedAjaxQueue({
            ...
            success: function (data) {
              ...
              currentTimesBoxDetails.val(
                JSON.stringify(response.timeslot_details)
              );
      ...
      thisTimeslot.on("click", ".changeCount", function (e) {
        ...
        if (!currentlySaving) {
          ...
          if (newCount < 1) {
            ...
            if (newCount != 1) {
              slot_text = booked_js_vars.i18n_slots;
            } else {
              slot_text = booked_js_vars.i18n_slot;
            }
      ...
      thisTimeslot.on("click", ".addSingleTimeslot_button", function (e) {
        ...
        var addTimeslotsFormWrapper = $(
          "#booked-customTimePickerTemplates .customSingle"
        );

        var startTime = addTimeslotsFormWrapper
            .find("select[name=startTime]")
            .val(),
          startTimeText = addTimeslotsFormWrapper
            .find("select[name=startTime] :selected")
            .text(),
          endTime = addTimeslotsFormWrapper.find("select[name=endTime]").val(),
          endTimeText = addTimeslotsFormWrapper
            .find("select[name=endTime] :selected")
            .text(),
          count = addTimeslotsFormWrapper.find("select[name=count]").val(),
          countText = addTimeslotsFormWrapper
            .find("select[name=count] :selected")
            .text(),
          title = addTimeslotsFormWrapper.find("input[name=title]").val(),
          calendar_id = addTimeslotsFormWrapper
            .parents(".booked-customTimeslot")
            .find('[name="booked_custom_calendar_id"]')
            .val(),
          start_date = addTimeslotsFormWrapper
            .parents(".booked-customTimeslot")
            .find('[name="booked_custom_start_date"]')
            .val(),
          end_date = addTimeslotsFormWrapper
            .parents(".booked-customTimeslot")
            .find('[name="booked_custom_end_date"]')
            .val(),
          currentTimesBox = addTimeslotsFormWrapper
            .parents(".booked-customTimeslot")
            .find("input[name=booked_this_custom_timelots]"),
          currentTimes = addTimeslotsFormWrapper
            .parents(".booked-customTimeslot")
            .find("input[name=booked_this_custom_timelots]")
            .val(),
          currentTimesDetailsBox = addTimeslotsFormWrapper
            .parents(".booked-customTimeslot")
            .find("input[name=booked_this_custom_timelots_details]"),
          currentTimesDetails = addTimeslotsFormWrapper
            .parents(".booked-customTimeslot")
            .find("input[name=booked_this_custom_timelots_details]")
            .val(),
          timeslotList = addTimeslotsFormWrapper
            .parents(".booked-customTimeslot")
            .find(".customTimeslotsList");

        var formData = addTimeslotsFormWrapper
          .parents(".booked-customTimeslot")
          .find("#single-timeslot-form")
          .serializeObject();
        ...
        if (startTime && endTime && count) {
          ...
          if (
            ...
            (startTime == "allday" && endTime == "2400")
          ) {
            appt_add_confirm = confirm(
              booked_js_vars.i18n_single_add_confirm +
                ":\n" +
                booked_js_vars.i18n_all_day +
                " x" +
                count
            );
          } else {
      ...
      thisTimeslot.on("click", ".addBulkTimeslots_button", function (e) {
        ...
        var addTimeslotsFormWrapper = $(
          "#booked-customTimePickerTemplates .customBulk"
        );
        ...
        var startTime = addTimeslotsFormWrapper
          ...
          endTimeText = addTimeslotsFormWrapper
            .find("select[name=endTime] :selected")
            .text(),
          interval = addTimeslotsFormWrapper
            .find("select[name=interval]")
            .val(),
          time_between = addTimeslotsFormWrapper
            .find("select[name=time_between]")
            .val(),
          intervalText = addTimeslotsFormWrapper
            .find("select[name=interval] :selected")
            .text(),
          count = addTimeslotsFormWrapper.find("select[name=count]").val(),
          countText = addTimeslotsFormWrapper
            .find("select[name=count] :selected")
    ...
    function loadCustomTimeSlots(timeslotsBlock, json_array) {
      var parentTimeslotBlock = timeslotsBlock.parents(
          ".booked-customTimeslot"
        ),
        start_date = parentTimeslotBlock
          .find('[name="booked_custom_start_date"]')
          .val(),
        end_date = parentTimeslotBlock
          .find('[name="booked_custom_end_date"]')
          .val(),
        calendar_id = parentTimeslotBlock
          .find('[name="booked_custom_calendar_id"]')
          .val(),
      ...
      $.ajax({
        ...
        type: "post",
        data: {
          action: "booked_admin_custom_timeslots_list",
          nonce: booked_js_vars.nonce,
          json_array: JSON.stringify(json_array_jsoned.timeslot),
          json_array_detailed: JSON.stringify(
            json_array_jsoned.timeslot_details
          ),
          start_date: start_date,
          end_date: end_date,
          calendar_id: calendar_id,
    ...
    $("#booked-custom-timeslots").on(
      ...
      function (e) {
        ...
        booked_js_vars.ajaxRequests.push = $.bookedAjaxQueue({
          ...
          success: function (data) {
            $("#customTimeslotsContainer .booked-customTimeslot").css(
              "border-color",
              "#ddd"
            );
            $("#booked-saveCustomTimeslots")
    ...
    $("body").on("click", ".booked-cf-block .cfButton", function (e) {
      ...
      var newSortable = CF_SortablesTemplatesContainer.find(
        "#bookedCFTemplate-" + CF_ButtonType
      )
        .clone()
      ...
      if (CF_ButtonType == "plain-text-content") {
        var thisInput = newSortable.find(
          'textarea[name="' + CF_ButtonType + '"]'
        );
      ...
      if (CF_ButtonType != "plain-text-content") {
        thisRequiredCheckbox
          .attr("name", "required---" + randomNumber)
          .attr("id", "required---" + randomNumber)
          .parent()
          .find("label")
          .attr("for", "required---" + randomNumber);
      }
      ...
      if (CF_ButtonType == "single-paid-service") {
        ...
        var options = CF_SortablesTemplatesContainer.find(
          "#bookedCFTemplate-single-paid-service select option"
        );
    ...
    $("body").on(
      "change",
      ".booked-cf-block .cf-required-checkbox",
      function (e) {
      ...
      function (e) {
        ...
        booked_js_vars.ajaxRequests.push = $.bookedAjaxQueue({
          ...
          success: function (data) {
            ...
            $("#booked-cf-sortables input[type=text]").css(
              "border-color",
              "#ccc"
            );
      ...
      adminTabs.find("li > a").on("click", function (e) {
        ...
        if (activeTab == "custom-fields") {
          window.location.replace(
            "/wp-admin/admin.php?page=booked-settings#custom-fields"
          );
          location.reload();
        } else {
          $("#booked-" + activeTab).show();
    ...
    function reset_add_timeslot_button() {
      var addButton = $("#bookedTimeslotsWrap")
        .find(".addTimeslot.active")
        .find("a.booked-add-timeslot");
      var dayName = addButton.parent().attr("data-day");
      var clearButton = $(
        'table.booked-timeslots th[data-day="' + dayName + '"]'
      ).find(".booked-clear-timeslots");
      $("td.addTimeslot").find("select").show();
      $("td.addTimeslot").removeClass("active");
      $("td.addTimeslot a.button")
        .html(booked_js_vars.i18n_add)
        .removeClass("button-primary")
        .appendTo('table.booked-timeslots th[data-day="' + dayName + '"]');
      $("#timepickerTemplate").appendTo("#booked-defaults").hide();
      clearButton.show();
    }

    $(
      "#timepickerTemplate .tsSingle,#booked-customTimePickerTemplates .customSingle"
    ).on("change", 'select[name="startTime"]', function () {
    ...
    $("#bookedTimeslotsWrap").on(
      "click",
      "a.booked-clear-timeslots",
      function (e) {
        e.preventDefault();
        var thisButton = $(this);
        var dayName = thisButton.parent().attr("data-day");
        var parentBlock = $("#bookedTimeslotsWrap").find(
          'td.addTimeslot[data-day="' + dayName + '"]'
        );
        var day = thisButton.parents("th").attr("data-day");
        var calendar_id = $("table.booked-timeslots").attr("data-calendar-id");

        clear_timelots_confirm = confirm(
          booked_js_vars.i18n_clear_timeslots_confirm
        );
        if (clear_timelots_confirm == true) {
          $.ajax({
            ...
            type: "post",
            data: {
              action: "booked_admin_clear_timeslots",
              nonce: booked_js_vars.nonce,
              day: day,
              calendar_id: calendar_id,
            },
            ...
            success: function (data) {
              ...
              $.ajax({
                ...
                type: "post",
                data: {
                  action: "booked_admin_load_timeslots",
                  nonce: booked_js_vars.nonce,
                  day: day,
                  calendar_id: calendar_id,
                },
        ...
        if (thisButton.hasClass("button-primary")) {
          ...
          if (activeTab == "#Bulk") {
            ...
            var $button = $(this),
              ...
              calendar_id = $("table.booked-timeslots").attr(
                "data-calendar-id"
              ),
              countText = $(".tsBulk")
                .find("select[name=count] :selected")
            ...
            var $button = $(this),
              ...
              startTime = $(".tsSingle")
                .find("select[name=startTime] :selected")
                .val(),
              startTimeText = $(".tsSingle")
                .find("select[name=startTime] :selected")
                .text(),
              endTime = $(".tsSingle")
                .find("select[name=endTime] :selected")
                .val(),
              endTimeText = $(".tsSingle")
                .find("select[name=endTime] :selected")
                .text(),
              //Producst
              productId = $(".tsSingle")
                .find("select[name=product] :selected")
                .val(),
              productName = $(".tsSingle")
                .find("select[name=product] :selected")
                .attr("data-product-name"),
              productPrice = $(".tsSingle")
                .find("select[name=product] :selected")
    ...
    $("#booked-timeslotsSwitcher").on(
      "change",
      'select[name="bookedTimeslotsDisplayed"]',
      function (e) {
        ...
        $.ajax({
          ...
          type: "post",
          data: {
            action: "booked_admin_load_full_timeslots",
            nonce: booked_js_vars.nonce,
            calendar_id: calendar_id,
          },
          success: function (html) {
    ...
    $("#booked-cfSwitcher").on(
      "change",
      'select[name="bookedCustomFieldsDisplayed"]',
      function (e) {
        var calendar_id = $(this).val();
        ...
        $.ajax({
          ...
          type: "post",
          data: {
            action: "booked_admin_load_full_customfields",
            nonce: booked_js_vars.nonce,
            calendar_id: calendar_id,
          },
          success: function (html) {
    ...
    // Calendar Switcher
    $(".booked-calendarSwitcher").on(
      "change",
      'select[name="bookedCalendarDisplayed"]',
      function (e) {
        var calendar_id = $(this).val(),
        ...
        $.ajax({
          ...
          type: "post",
          data: {
            action: "booked_admin_calendar_month",
            nonce: booked_js_vars.nonce,
    ...
    $("body").on(
      "click",
      ".booked-admin-calendar-wrap .page-right, .booked-admin-calendar-wrap .page-left, .booked-admin-calendar-wrap .monthName a",
      function (e) {
        e.preventDefault();
    ...
    $(".booked-admin-calendar-wrap").on(
      "click",
      "tr.entryBlock .delete",
      function (e) {
        e.preventDefault();
    ...
    $(".booked-admin-calendar-wrap").on(
      ...
      function (e) {
        ...
        confirm_appt_approve = confirm(
          booked_js_vars.i18n_confirm_appt_approve
        );
    ...
    $("body").on(
      "click",
      ".booked-pending-appt-list .user, .booked-admin-calendar-wrap tr.entryBlock .user",
      function (e) {
        e.preventDefault();
        ...
        $.ajax({
          ...
          type: "post",
          data: {
            action: "booked_admin_user_info_modal",
            nonce: booked_js_vars.nonce,
            user_id: user_id,
            appt_id: appt_id,
          },
    ...
    $(".booked-admin-calendar-wrap").on(
      ...
      function (e) {
        ...
        booked_load_calendar_date_booking_options = {
          action: "booked_admin_new_appointment_form",
          nonce: booked_js_vars.nonce,
          date: date,
          title: title,
          timeslot: timeslot,
    ...
    $(".booked-admin-calendar-wrap").on(
      "click",
      "tr.entryBlock button.disable-slot",
      function (e) {
        e.preventDefault();
        ...
        booked_load_calendar_date_booking_options = {
          action: "booked_admin_disable_slot",
          nonce: booked_js_vars.nonce,
          date: date,
          title: title,
          timeslot: timeslot,
    ...
    $(".booked-pending-appt-list").on(
      ...
      function (e) {
        ...
        confirm_appt_delete = confirm(booked_js_vars.i18n_confirm_appt_delete);
        if (confirm_appt_delete == true) {
          var currentPendingCount = parseInt(
            $("li.toplevel_page_booked-appointments")
              .find("li.current")
              .find("span.update-count")
              .html()
          );
    ...
    $(".booked-pending-cap").on("click", ".approve-all", function (e) {
      ...
      confirm_appt_approve = confirm(
        booked_js_vars.i18n_confirm_appt_approve_all
      );
      if (confirm_appt_approve == true) {
        savingState(true);
        ...
        booked_js_vars.ajaxRequests.push = $.bookedAjaxQueue({
          ...
          success: function (data) {
            $("li.toplevel_page_booked-appointments")
              ...
              .remove();
            $(
              ".booked-pending-appt-list .pending-appt:not(.no-pending-message)"
            ).remove();
            $(".booked-pending-appt-list .no-pending-message").slideDown(
              "fast"
            );
            $(".booked-pending-cap").slideUp("fast");
            setTimeout(function () {
    ...
    $(".booked-pending-cap").on("click", ".delete-all", function (e) {
      ...
      confirm_appt_approve = confirm(
        booked_js_vars.i18n_confirm_appt_delete_all
      );
      if (confirm_appt_approve == true) {
        savingState(true);
    ...
    var totalPendings = $(".booked-pending-appt-list").find(
      ".pending-appt"
    ).length;
    var passedPendings = $(".booked-pending-appt-list").find(
      ".pending-appt.passed"
    ).length;
    ...
    $(".booked-pending-cap").on("click", ".delete-past", function (e) {
      ...
      confirm_appt_approve = confirm(
        booked_js_vars.i18n_confirm_appt_delete_past
      );
      if (confirm_appt_approve == true) {
        var currentPendingCount = parseInt(
          $("li.toplevel_page_booked-appointments")
        ...
        var pendingsToRemove = $(".booked-pending-appt-list").find(
          ".pending-appt.passed"
        ).length;
    ...
    $("body").on(
      "touchstart click",
      ".bm-overlay, .bm-window .close, form.booked-form .cancel",
      function (e) {
        e.preventDefault();
        close_booked_modal();
        return false;
    ...
    $("body").on(
      "change",
      "form.booked-settings-form input,form.booked-form input,form.booked-settings-form select",
      function () {
    ...
    function savingState(show, savingStateObj) {
      if (typeof savingStateObj === "undefined") {
        var savingStateObj = $(
          "li.active .savingState, .topSavingState.savingState, .calendarSavingState, .cf-updater.savingState, .cts-updater.savingState, .cal-updater.savingState"
        );
      }
    ...
    function updateCustomTimeslotEncodedField() {
      var formData = JSON.stringify(
        $("form#customTimeslots").serializeObject()
      );
...
function adjust_calendar_boxes() {
  ...
  jQuery(".booked-calendar tbody tr.week td .date").css(
    "line-height",
    boxesHeight + "px"
  );
